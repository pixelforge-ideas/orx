...
#define orxFONTGEN_KF_MIN_RANGE                     2.0f
...
static void GetFontShaderCode(orxCHAR* pacBuffer, orxU32 u32BufferSize)
{
  ...
  orxString_NPrint(pacBuffer, u32BufferSize, szShaderSource, sstFontGen.fPadding * 0.5f);
}
...
static void Run()
{
  ...
  if(orxFLAG_TEST_ALL(sstFontGen.u32Flags, orxFONTGEN_KU32_STATIC_MASK_READY))
  {
    ...
    if(u32Counter)
    {
      ...
      {
        if(sstFontGen.fPadding < orx2F(2.0f) * orxFONTGEN_KF_MIN_RANGE)
        {
          /* Updates padding */
          sstFontGen.fPadding = orx2F(2.0f) * orxFONTGEN_KF_MIN_RANGE;

          /* Logs message */
          orxFONTGEN_LOG(SDF, "Character padding (SDF pixel range) set to minimum of <%g>.", orxFONTGEN_KF_MIN_RANGE);
        }
      }
