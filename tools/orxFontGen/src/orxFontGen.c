...
#define orxFONTGEN_KU32_STATIC_FLAG_SDF             0x00000010  /**< SDF flag */
#define orxFONTGEN_KU32_STATIC_FLAG_RANGE           0x00000020  /**< Range flag */
...
#define orxFONTGEN_KF_DEFAULT_RANGE                 6.0f
...
static void GetFontShaderCode(orxCHAR* pacBuffer, orxU32 u32BufferSize)
{
  static const orxSTRING szShaderSource =
    ...
    "  gl_FragColor = vec4(mix(OutlineColor, gl_Color.rgb, outsideBorder), inside * gl_Color.a);\n"
    "}\n";

  orxString_NPrint(pacBuffer, u32BufferSize, szShaderSource, sstFontGen.fRange);
}
...
static orxSTATUS orxFASTCALL ProcessRangeParams(orxU32 _u32ParamCount, const orxSTRING _azParams[])
{
  orxSTATUS eResult;
  ...
  if (_u32ParamCount > 0)
  {
    ...
    // Gets it
    if ((eResult = orxString_ToFloat(_azParams[1], &fRange, orxNULL)) != orxSTATUS_FAILURE)
    {
      // Stores it
      sstFontGen.fRange = fRange;

      // Updates status
      orxFLAG_SET(sstFontGen.u32Flags, orxFONTGEN_KU32_STATIC_FLAG_RANGE, orxFONTGEN_KU32_STATIC_FLAG_NONE);

      // Logs message
      orxFONTGEN_LOG(RANGE, "Pixel range set to <%g>.", fRange);
    }
    else
    {
      // Logs message
      orxFONTGEN_LOG(RANGE, "Invalid pixel range found in '%s', aborting.", _azParams[1]);
    }
  }
  else
  {
    // Sets default pixel range
    sstFontGen.fRange = orxFONTGEN_KF_DEFAULT_RANGE;

    // Logs message
    orxFONTGEN_LOG(OUTPUT, "No valid output found, using default '%g'.", orxFONTGEN_KF_DEFAULT_RANGE);
  }
...
static orxSTATUS orxFASTCALL ProcessSdfParams(orxU32 _u32ParamCount, const orxSTRING _azParams[])
{
  ...
  if (_u32ParamCount > 0)
  {
    ...
    // Logs message
    orxFONTGEN_LOG(SDF, "Using MTSDF.");
  }
...
#define orxFONTGEN_DECLARE_PARAM(SN, LN, SD, LD, FN) {orxPARAM_KU32_FLAG_STOP_ON_ERROR, SN, LN, SD, LD, &FN},
  ...
  orxPARAM  astParamList[] =
  {
    ...
    orxFONTGEN_DECLARE_PARAM("r", "range", "Pixel range", "Specifies the width of the range around the shape between the minimum and maximum representable signed distance in shape units or distance field pixels, respectivelly.", ProcessRangeParams)
    orxFONTGEN_DECLARE_PARAM("sdf", "sdf", "Use MTSDF", "Use a multi-channel signed distance field (MSDF) combined with a true SDF in the alpha channel", ProcessSdfParams)
...
static void Run()
{
  ...
  if(orxFLAG_TEST_ALL(sstFontGen.u32Flags, orxFONTGEN_KU32_STATIC_MASK_READY))
  {
    ...
    if(u32Counter)
    {
      ...
      s32Width  = orxF2S(fWidth * (sstFontGen.vCharacterSize.fX + sstFontGen.fPadding + sstFontGen.fRange) + (sstFontGen.vCharacterSpacing.fX + sstFontGen.fRange) * orxMAX(fWidth - orxFLOAT_1, orxFLOAT_0));
      s32Height = orxF2S(fHeight * (sstFontGen.vCharacterSize.fY + sstFontGen.fPadding + sstFontGen.fRange) + (sstFontGen.vCharacterSpacing.fY + sstFontGen.fRange) * orxMAX(fHeight - orxFLOAT_1, orxFLOAT_0));
      ...
      // Logs messages
      orxFONTGEN_LOG(PROCESS, "Calculated character size:    %4g x %g.", sstFontGen.vCharacterSize.fX + sstFontGen.fPadding + sstFontGen.fRange, sstFontGen.vCharacterSize.fY + sstFontGen.fPadding + sstFontGen.fRange);
      ...
      if(pu8ImageBuffer)
      {
        ...
        const orxSTRING azParamList[4] =
        {
          "Tex", "Thickness", "Outline", "OutlineColor"
        };
